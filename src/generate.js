...
- Adapter output is opt-in; \`codex-prep prepare\` does not generate external adapter files by default.
...
Use \`codex-prep orient --task "<task>" --profile standard\` first to get a compact reading list, then use \`codex-prep graph-query --file <path>\` for imports, dependents, symbols, and likely related tests. Use \`short\`, \`standard\`, and \`deep\` profiles to scale context deliberately.
...
- Adapters: ` + inlineValue(adapterStateSummary(adapters)) + `
- Handoff: ` + inlineValue(handoffStateSummary(handoff)) + `
...
function handoffStateSummary(handoff) {
  if (!handoff?.exists) {
    return "missing";
  }
  return handoff.stale ? "stale" : "fresh";
}
